...
Ticket Writer помогает готовить Jira-ready описания задач для проекта GTP: уточняет размытый scope, отделяет исследование от реализации, формулирует проверяемые AC и выдаёт markdown, который можно вставить в Jira. Поддерживаются две платформы:
